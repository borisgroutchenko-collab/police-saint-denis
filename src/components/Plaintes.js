...
function VerbalisationModal({ plainte, misIndex, agents, onClose, onDone, showNotif }) {
  ...
  async function submit() {
    ...
    try {
      ...
      // Saisies
      const saisieBase = { source: 'plainte', date: rpDate, heure: now.getHours().toString().padStart(2,'0') + ':' + now.getMinutes().toString().padStart(2,'0'), agent: agent || '', nomComplet: nomC, createdAt: serverTimestamp() };
